#!/usr/bin/env python3
"""
Simple script to generate display names for car database
Processes in small batches and saves frequently
"""
...
def main():
    ...
    for i in range(0, total_cars, batch_size):
        ...
        print(f"\nProcessing batch {i//batch_size + 1}: cars {i+1}-{end_idx}")
        
        for j, (idx, car_data) in enumerate(batch_df.iterrows()):
            if pd.isna(car_data['display_name']) or car_data['display_name'] == '':
                print(f"  Processing car {i+j+1}/{total_cars}: {car_data['Make']} {car_data['Model']}")
        ...
        print(f"  Batch complete. Processed {min(processed, total_cars)}/{total_cars} cars")
        
        # Save progress every batch
        print(f"  Saving progress...")
        df.to_excel('db2_clean_embedded_with_display_names_simple.xlsx', index=False)
        print(f"  Progress saved!")
        
        # Continue processing all cars
        # if i >= 50:  # Process first 60 cars for testing
        #     print(f"\nStopping after {i + batch_size} cars for testing...")
        #     break
    
    print(f"\nCompleted! Processed {min(processed, total_cars)} cars")
    print(f"Results saved to db2_clean_embedded_with_display_names_simple.xlsx")
