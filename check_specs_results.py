...
# Check the results
df = pd.read_excel('database_2.0_specs_complete.xlsx')

print('Sample of results:')
print(df[['Make', 'Model', 'Min. of mpg low helper', 'Min. of Insurance group', 'Min. of 0-60 mph (secs)']].head(10))

print(f'\nStill missing MPG: {(df["Min. of mpg low helper"] == 0).sum()}')
print(f'Still missing Insurance: {(df["Min. of Insurance group"] == 0).sum()}')
print(f'Still missing 0-60: {(df["Min. of 0-60 mph (secs)"] == 0).sum()}')
